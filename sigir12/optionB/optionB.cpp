...
{
    ifstream fin;
    fin.open("/home/xin_jin/projects/all_A_fragment/phase1/lucene_step/search_frag.txt");
    search_frag.clear();
    int fid, pos, term;
    int previous_term=-1;
...
{
    ifstream fin;
    fin.open("/home/xin_jin/projects/all_A_fragment/phase1/gen_index/forward.txt");
            ...
            for (int k=0; k<query_len; k++) // each time, deal with one term
            {
                ...
                if (index != -1){
                    ...
                    for (int l=0; l<vpos->size(); l++)
                        (*e)[k].push_back((*vpos)[l]+frag_offset);
                }
            }
        }
        vid_posting[vid] = e;
