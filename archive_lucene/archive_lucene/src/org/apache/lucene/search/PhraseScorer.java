...
import org.apache.lucene.document.Document;//my
import org.apache.lucene.index.IndexReader;//my
...
abstract class PhraseScorer extends Scorer {
  ...
  //my
  public void print(TermPositions[] tps, IndexReader reader)
  {
	  int total=0;
	  PhrasePositions current=first;
	  try{
		  BufferedWriter fout = new BufferedWriter(new FileWriter("search_frag.txt",false));
		  for (int i=0; i<tps.length; i++)
		  {
			  ...
			  while(current.next())
			  {
				  ...
				  Document doc = reader.document(current.doc);
				  String path = doc.get("path");
				  //System.out.print((++total)+" term "+i+" "+path+" pos "+(current.position+current.offset));
				  String[] split_words;
				  split_words = path.split("/");
				  String fragment_word = split_words[split_words.length-1];
				  split_words = fragment_word.split("\\.");
				  fout.write(i+" "+split_words[0]+" "+(current.position+current.offset));
				  while(current.nextPosition())
				  {
					  //System.out.print(" pos "+(current.position+current.offset));
					  fout.write(" "+(current.position+current.offset));
				  }
				  //System.out.println();
				  fout.write(" -1\n");
			  }		  
  ...
  PhraseScorer(Weight weight, TermPositions[] tps, int[] offsets,
      Similarity similarity, byte[] norms, IndexReader reader) {
    super(similarity);
    this.norms = norms;
    this.weight = weight;
