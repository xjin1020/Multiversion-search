...
// check if this cluster's "search_frag.txt" contains all terms
// if no, output an empty "result.txt" and exit 
void check_contain_all_terms(int q_len)
{
    ifstream fin;
    fin.open("search_frag.txt");
    int s=-1;
    bool flag=true; // if false, output an empty result and exit
    int termid;
    ...
    if (flag == false)
    {
        ...
        fout.close();
        cout << "search_frag.txt does not contain all query terms, disregard it" << endl;
        exit(1);
    }
}

void read_search_frag(int q_len)
{
    check_contain_all_terms(q_len);
    ifstream fin;
    fin.open("search_frag.txt");
    search_frag.clear();
    int fid, pos, term;
...
int main(int argc, char** argv)
{
    if (argc != 2)
    {
        cout << "Usage: need one parameter query_length" << endl;
        exit(1);
    }
    int query_len = atoi(argv[1]);
    init();
    // move read_search_frag() before read_index() because read_search_frag() might terminate if the input search_frag.txt does not satisfy requirements, then we don't need to read index. save time.
    read_search_frag(query_len);
    read_index();
    intersection();
    scoring();
	// Susen
