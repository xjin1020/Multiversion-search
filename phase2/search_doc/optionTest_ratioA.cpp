...
//new binary search with [start,end]
...
int MakeChoice(int k)
{
	...
	/*
        if(f<forward_table[vid_list[i]].size()){
            frag_sum +=  f*(1+log2(forward_table[vid_list[i]].size()/f));
        }
        else{
            frag_sum +=  forward_table[vid_list[i]].size()*(1+log2(f/forward_table[vid_list[i]].size()));
        }
	*/
    }
    int pi = frag_sum/m;
    //bool res = (f*u*p)<=(m*pi*(log2(f)+p));
    //int res = frag_sum*(1+(int)log2(f));
    int res = doc_sum;
    //cout << f << " "<< u << " "<< p<< " "<< m << " "<< pi<<endl;
    //cout << f*u*p << " "<<m*pi*(log2(f)+p)<<endl;
    //cout <<f*u*(7+p) <<" "<<m*pi*(log2(f)+p+2)<<endl;
    return res;
}
...
void get_positional_info()
{
	...
	int counter = MakeChoice(k);
        cout << "Calculate Term " << k << " : " << dwtime << " ms f= " << f << endl;
	cout << "Counter value "<< counter << " Term " << k << endl;
    }
    cout << "!!" << endl;
    cout << endl;
