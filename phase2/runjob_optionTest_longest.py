...
query_len = 6#!!!USER!!!
fin = open('../phase1_long/search_doc/vidlist.txt', 'r')# !!!USER!!!use phase 1's result, need to modify     input file path
k=1000#!!!USER!!! top k results in option B, we will find p clusters which contain k pages
latest_offset = 3532308 ##!!!USER!!! 3532308 reason: we use latest version, so here vid 0 is actually the first vid of the last version. How to compute this number: the last vid of vdrelation.txt + 1 - number of lines in "phase1_long/fragment/data/rep.txt"

line = fin.readline()
fin.close()
vd_list = line.rstrip('\n').split(' ')
vd_list.remove('') # vd_list contains the vid from phase1_long
...
for p in range(len(vd_list)):
    ...
    com = '../../search_doc/optionTest '+ str(query_len)
    print 'run optionTest:'
    run_command(com)
