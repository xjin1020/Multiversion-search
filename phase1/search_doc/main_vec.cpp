...
unordered_map<int, vector<vector<int>>> intersection_hash; // this keeps the intersection results
...
void intersection1()
{
    ...
    clock_t start_sort, start_insert, start_merge;
    double duration_sort=0, duration_insert=0, duration_merge=0;
    ...
    for (i=0; i<term_number; i++) // deal with each query term
    {
        ...
        duration_sort += (clock() - start_sort) / (double) CLOCKS_PER_SEC;
        ...
        for (j=1; j<current_posting.size(); j++)
        {
            ...
            else // merge
            {
                 ...
                 */
                int index_oldpos = 0;
                vector<int> old_pos = (*merge_result)[pre_index].pos;
                vector<int> new_pos;
                bool flag = false; // if old_pos's elements has used up
                for (iter = current_posting[j].pos.begin(); iter != current_posting[j].pos.end();)
                {
                    if (flag || *iter < old_pos[index_oldpos]){
                        new_pos.push_back(*iter);
                        iter++;
                        continue;
                    }
                    else if (*iter > old_pos[index_oldpos]){
                        new_pos.push_back(old_pos[index_oldpos]);
                        index_oldpos++;
                    }
                    else{ // *iter == old_pos[index_oldpos]
                        new_pos.push_back(*iter);
                        index_oldpos++;
                    }
                    if (index_oldpos == old_pos.size())
                        flag = true;
                }
                while (iter != current_posting[j].pos.end()) // some elements not inserted
                {
                    new_pos.push_back(*iter);
                    iter++;
                }
                (*merge_result)[pre_index].pos = new_pos;
            }
        }
        duration_merge += (clock() - start_merge) / (double) CLOCKS_PER_SEC;
    }
    
    duration = (clock() - start) / (double) CLOCKS_PER_SEC;
    cout << "Step1_insert:" << duration_insert*1000 << "ms" << endl;
    cout << "Step1_sort:" << duration_sort*1000 << "ms" << endl;
    cout << "Step1_merge:" << duration_merge*1000 << "ms" << endl;
    cout << "Step1:" << duration*1000 << "ms"<< endl;
    ...
    // end timing
    duration = (clock() - start) / (double) CLOCKS_PER_SEC;
    cout << "Step1+Step2:" << duration*1000 << "ms"<< endl;
...
void intersection()
{
    ...
    for (i=0; i<v_occ.size(); i++) // each time, deal with one fragment occurence
    {
        ...
        for (j=0; j<v_reuse.size(); j++) // each time deal with a reuse version page for a given fragment
        {
            ...
            unordered_map<int, vector<vector<int>>>::iterator iter = intersection_hash.find(vid);
            vector<vector<int>> *p; // pointer of the value of intersection_hash[vid]
            ...
            {
                vector<vector<int>> new_entry; new_entry.clear();
            ...
            vector<int> *pos_vector = &((*p)[0]);
            for (m=0; m<(o->v_pos).size(); m++) // loop for each position in the fragment occurence
                pos_vector->insert(o->v_pos[m]+frag_offvector); // insert into the vector; thus if duplicate, will not save
        }
    }
    ...
    {
        unordered_map<int, vector<vector<int>>> temp_intersection_hash;
        ...
        for (i=0; i<v_occ.size(); i++) // each time, deal with one fragment occurence
        {
            ...
            for (j=0; j<v_reuse.size(); j++)
            {
                ...
                unordered_map<int, vector<vector<int>>>::iterator iter_temp = temp_intersection_hash.find(vid);
                if (iter_temp == temp_intersection_hash.end()) // not find, copy the entry from intersection_hash and create a new vector for this term
                {
                    vector<vector<int>> cp_entry = *p;
                    vector<int> t; t.clear();
                    cp_entry.push_back(t);
                    temp_intersection_hash[vid] = cp_entry;
                }
                // if find, don't need to do anything because the new vector for this term should be created before
                ...
                vector<int> *pos_vector = &((*p)[term]);
                for (m=0; m<(o->v_pos).size(); m++)
                {
                    pos_vector->insert((o->v_pos)[m]+frag_offset);
                }
            }
        }
...
void cal_min_span_title(int &min_span, int vid, vector<vector<int>> &occur_terms, int term_number, int title_len)
{
    ...
    // copy occur_terms to vec_occ because vector cannot be sorted
...
void cal_min_span(int &min_span, int vid, vector<vector<int>> &occur_terms, int term_number)
{
    // cur_pos saves the current focus of terms' positions in the algorithm
    int *cur_pos = new int[term_number];
    int *cur_index = new int[term_number];
...
int body_tf(int vid, vector<vector<int>> &occur_terms, int term_number)
{
    if (term_number != occur_terms.size())
    {
...
void score_page(int vid, vector<vector<int>> &occur_terms)
{
    ...
    for (int i=0; i<term_number; i++)
    {
        ...
        for (iter_vector = occur_terms[i].begin(); iter_vector != occur_terms[i].end(); iter_vector++)
        {
            if (*iter_vector < title_len && if_cal_appear == false)
            {
                appear++;
                if_cal_appear = true;
            }
