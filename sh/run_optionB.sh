...
if [ -z "$1" ]; then
    ...
    cd ../../phase2
    # edit query
    sed -i "s/query_len = .*#!!!/query_len = $2#!!!/g" runjob_optionBpre.py
    sed -i "s/k=.*#!!!/k=$top_k#!!!/g" runjob_optionBpre.py
    ...
    echo
    echo ">>>> Start Searching using OptionBpre <<<<"
    echo
    
    python runjob_optionBpre.py &> outputBpre

    # Step 4: copy "vidlistBpre.txt" to phase1/search_doc to run optionB
    # This copy is because we want to test TP with the exact same vids as RTP for fair comparison
    cp vidlistBpre.txt ../phase1/search_doc/vidlist.txt
    # edit query
