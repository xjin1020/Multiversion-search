...
if [ -z "$1" ]; then
    ...
    cd ../../phase2
    # edit query
    sed -i "s/query_len = .*#!!!/query_len = $2#!!!/g" runjob_intersection_longest.py
    sed -i "s/k=.*#!!!/k=$top_k#!!!/g" runjob_intersection_longest.py
    ...
    echo
    echo ">>>> Start Searching using Intersection <<<<"
    echo
    python runjob_intersection_longest.py &> outputIntersection
    python calculateInter.py
