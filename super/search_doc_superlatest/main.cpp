...
unordered_map<int, int> vdrelation;
...
{
    ifstream fin;
    fin.open("../../phase1_rep/gen_index/index.txt");
...
{
    ifstream fin;
    fin.open("../../phase1_rep/lucene_step/search_frag.txt");
    search_frag.clear();
    int fid, pos, term;
    int previous_term=-1;
...
{
    int did = vdrelation[vid+3532308];
...
void read_superinfo()
{
    //(1) read vdrelation
    ifstream fin;
    fin.open("../../phase1/gen_index/vdrelation.txt");
    int vid, did;
    while(fin >> vid >> did)
    {
        vdrelation[vid] = did;
