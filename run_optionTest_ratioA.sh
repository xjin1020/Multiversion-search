...
if [ -z "$1" ]; then
    ...
    cd ../../phase2
    # edit query
    sed -i "s/query_len = .*#!!!/query_len = $2#!!!/g" runjob_optionTest_ratioA.py
    sed -i "s/k=.*#!!!/k=$top_k#!!!/g" runjob_optionTest_ratioA.py
    ...
    sed -i "s/query_len = .*#!!!/query_len = $2#!!!/g" calculateTest_ratio.py

    echo
    echo ">>>> Start Searching using OptionTest <<<<"
    echo
    python runjob_optionTest_ratioA.py &> outputTest
    python calculateTest_ratio.py
